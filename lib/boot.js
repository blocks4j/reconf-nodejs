...
settings.properties.forEach(property => {
    ...
    Request(src).then(propertyValue => setReconfMethod(property, propertyValue));
});

function setReconfMethod(property, propertyValue) {
    console.log(`[Remote Config] Updated property => ${property}`);

    let methodName = `get${property.toPascalCase()}`;
    Reconf.set(methodName, () => { return propertyValue });
}
